<template>
	<view>
		<view class="icon-form">
			<view class="icon-col active">
				<view class="icon iconfont" v-html="activeValue"></view>
			</view>
			<input type="text" :value="categoryName" placeholder="输入类别名称(不超过4个汉字)" @input="hanleCategoryName" />
...
export default {
	...
	methods: {
		...
		hanleCategoryName(event){
			const categoryName = event.detail.value.trim()
			this.categoryName =	categoryName.length > 4? '':categoryName
		},
		submit(){
			...
			this._db.collection('Category').add({
				data:{
					type,
					icon,
					name
